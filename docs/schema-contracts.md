...
The Phase 4 application project contract is additive `2.4.0` (software/API `0.4.0`). Its migration accepts `1.0.0`, `2.0.0`, `2.1.0`, `2.2.0`, and `2.3.0`, preserves all earlier source/configuration/geometry/quarantine data, and creates no inferred calculation areas. `calculation_areas` and `lighting_calculations` are separate from camera `priority_areas`. The IES operational contract is `1.2.0` with immutable file history and project pins to exact IES revisions. Exact in-memory freshness tests cover project schema, OpenAPI, and operational catalogs.
