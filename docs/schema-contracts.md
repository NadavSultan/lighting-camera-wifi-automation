...
The corrective Phase 3 application project contract is additive `2.3.0` (software/API `0.3.1`). Its migration accepts `1.0.0`, `2.0.0`, `2.1.0`, and `2.2.0`, preserves source coordinates and legacy orientation bytes, and never guesses Phoenix 1 versus Solitaire. Footprints persist exact H/V FOV and mounting `geometry_contract_version`; priority summaries expose warnings in project schema and OpenAPI. Invalid legacy `2.2.0` priority records are preserved losslessly in `legacy_invalid_priority_areas` and excluded until explicitly redrawn. Exact in-memory freshness tests cover project schema and OpenAPI as well as the operational catalogs.
