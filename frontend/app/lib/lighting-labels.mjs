...
/**
 * Display lux with unit. Null/unavailable stays an em dash without a unit.
 * Does not rewrite stored lux values.
 * @param {unknown} value maintained lux
 * @returns {string}
 */
export function formatLuxWithUnit(value) {
  const formatted = formatLux(value);
  return formatted === "—" ? "—" : `${formatted} lx`;
}
