...
export interface CapPlanningResult {
  model_version: "jnet1-graph-planning-1.0.0"; projected_crs: string; disclaimer: string; heuristic: string;
  selected_candidate_ids: string[];
  assignments: Array<{ node_id: string; gateway_id: string; parent_id: string; hop: number; distance_m: number }>;
  canonical_links: Array<{ id: string; left_id: string; right_id: string; distance_m: number }>;
  node_snapshots: Array<{ id: string; kind: "fixture_node"; source_pole_id: string | null; candidate_id: null; projected_x_m: number; projected_y_m: number }>;
  candidate_snapshots: Array<{ id: string; kind: "gateway_root"; source_pole_id: string | null; candidate_id: string | null; projected_x_m: number; projected_y_m: number }>;
  unresolved_node_ids: string[];
  objective_trace: Array<{ candidate_id: string; marginal_serviceable_nodes: number; priority: number }>;
  limits: Record<string, number>; provenance: Record<string, unknown>; warnings: string[]; result_sha256: string;
}
