...
export function PoleInspector({ pole, project, fixtureCatalog, cameraCatalog, iesLibrary, onAssignModel, onChange, onRestore }: { pole: EffectivePole; project: Project; fixtureCatalog: FixtureModelCatalog | null; cameraCatalog: CameraEquipmentCatalog | null; iesLibrary: IesLibrary | null; onAssignModel: (modelId: string) => void; onChange: (patch: Partial<PoleEdit>) => void; onRestore: () => void }) {
  ...
  return <>
    ...
    {model?.capabilities.wifi && config && <section className="section"><div className="section-heading"><h3>Wi-Fi configuration</h3><span className="phase-tag">Phase 2</span></div><div className="field full"><label htmlFor="wifi-notes">Configuration notes</label><input id="wifi-notes" value={String(config.wifi_configuration?.notes ?? "")} onChange={(event) => updateConfig({ wifi_configuration: { ...(config.wifi_configuration ?? {}), notes: event.target.value } })} /></div><p className="helper">Configuration only; conceptual RF coverage remains deferred to Phase 5.</p></section>}
    ...
    <section className="section"><div className="future-card"><strong>Phase 4 lighting is available through Calculation Areas.</strong><br />Conceptual Wi-Fi, CAP recommendation, reporting, and automatic pole placement remain gated.</div><p className="helper">Lighting requires an explicit compatible IES revision, mounting height, and fixture azimuth. Conceptual Wi-Fi default retained: {project.defaults.wifi_radius_m} m.</p></section>
  </>;
}
