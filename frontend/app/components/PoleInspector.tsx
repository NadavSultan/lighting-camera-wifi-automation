...
export function PoleInspector({ pole, project, fixtureCatalog, cameraCatalog, iesLibrary, onAssignModel, onChange, onRestore }: { pole: EffectivePole; project: Project; fixtureCatalog: FixtureModelCatalog | null; cameraCatalog: CameraEquipmentCatalog | null; iesLibrary: IesLibrary | null; onAssignModel: (modelId: string) => void; onChange: (patch: Partial<PoleEdit>) => void; onRestore: () => void }) {
  ...
  const model = fixtureCatalog?.fixture_models.find((item) => item.id === config?.fixture_model_id);
  ...
  return <>
    ...
    <section className="section">
      ...
      {config && model && (config.fixture_model_revision !== model.revision || config.mounting_template_revision !== model.current_mounting_template_revision) && <button className="quiet-button restore-button" onClick={() => updateConfig({ fixture_model_revision: model.revision, mounting_template_revision: model.current_mounting_template_revision })}>Explicitly adopt current catalog/template revision</button>}
    </section>
